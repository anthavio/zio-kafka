...
object Serde extends Serdes {
   ...
   */
  def apply[R, T](
    deser: (String, Array[Byte]) => RIO[R, T]
  )(ser: (String, T) => RIO[R, Array[Byte]]): Serde[R, T] =
    new Serde[R, T] {
      override def serialize(topic: String, value: T): RIO[R, Array[Byte]] =
        ser(topic, value)
      override def deserialize(topic: String, data: Array[Byte]): RIO[R, T] =
        deser(topic, data)
